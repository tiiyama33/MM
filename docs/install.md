...
- 門馬綱一氏のVESTAのページ [http://jp-minerals.org/jp/](http://jp-minerals.org/jp/) (Shiftキーを押しながらクリックして別ウィンドウで開くとよい)
...
[Latest development versions]から自分のPCに合ったものをダウンロードします。  
...
2. 展開するフォルダーを聞かれるので、通常そのまま[展開]を選びます。
3. この状態でもプログラムは動きますが、正しいフォルダーに移動した方が良いでしょう。  
...
4. 移動した c:¥program files¥vesta 中にある、[VESTA.exe] がプログラム本体です。  
ダブルクリックして起動しましょう。  
...
> VESTA.exeではなく、単に VESTA と表示されている場合には、エクスプローラの上部メニューから [表示]-[表示]-[ファイル名拡張子] にチェックを入れると VESTA.exe と表示されるようになります。
